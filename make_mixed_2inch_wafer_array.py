...
OUTPUT_GDS = OUT_DIR / "mixed_radial_mla_2inch_wafer.gds"
...
def build_mixed_wafer() -> tuple[int, int]:
    ...
    lib.add(radial, mla)
    wafer = lib.new_cell("MIXED_RADIAL_MLA_2INCH_WAFER")
    ...
    for ix in range(-grid_limit, grid_limit + 1):
        for iy in range(-grid_limit, grid_limit + 1):
            ...
            # Checkerboard placement keeps the 2 die types spatially mixed.
            if (ix + iy) % 2 == 0:
